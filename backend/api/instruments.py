    ...
    try:
        ...
        return [
            RSScoreResponse(
                instrument_id=row.instrument_id,
                date=row.date,
                rs_line=Decimal(str(row.rs_line)) if row.rs_line is not None else None,
                rs_ma_150=Decimal(str(row.rs_ma_150)) if row.rs_ma_150 is not None else None,
                rs_trend=row.rs_trend,
                rs_pct_1m=Decimal(str(row.rs_pct_1m)) if row.rs_pct_1m is not None else None,
                rs_pct_3m=Decimal(str(row.rs_pct_3m)) if row.rs_pct_3m is not None else None,
                rs_pct_6m=Decimal(str(row.rs_pct_6m)) if row.rs_pct_6m is not None else None,
                rs_pct_12m=Decimal(str(row.rs_pct_12m)) if row.rs_pct_12m is not None else None,
                rs_composite=Decimal(str(row.rs_composite)) if row.rs_composite is not None else None,
                rs_momentum=Decimal(str(row.rs_momentum)) if row.rs_momentum is not None else None,
                volume_ratio=Decimal(str(row.volume_ratio)) if row.volume_ratio is not None else None,
                vol_multiplier=Decimal(str(row.vol_multiplier)) if row.vol_multiplier is not None else None,
                adjusted_rs_score=Decimal(str(row.adjusted_rs_score)) if row.adjusted_rs_score is not None else None,
    ...
    for i, d in enumerate(trading_dates):
        ...
        rs_records.append(
            RSScoreResponse(
                instrument_id=iid,
                date=d,
                rs_line=Decimal(str(round(100 + day_offset * 0.5, 4))),
                rs_ma_150=Decimal(str(round(100, 4))),
                rs_trend=rs_trend,
                rs_pct_1m=Decimal(str(round(pct_1m, 2))),
                rs_pct_3m=Decimal(str(round(pct_3m, 2))),
                rs_pct_6m=Decimal(str(round(pct_6m, 2))),
                rs_pct_12m=Decimal(str(round(pct_12m, 2))),
                rs_composite=Decimal(str(score)),
                rs_momentum=Decimal(str(momentum)),
                volume_ratio=Decimal(str(round(vol_raw, 3))),
                vol_multiplier=Decimal("1.000"),
                adjusted_rs_score=Decimal(str(score)),
                quadrant=quadrant,
